...
흐름(결정 E): SpatialIndex 반경 후보(C++) → numpy 코사인 top-K → 장소·예보지점 결합.
"""
...
def find_twins(source: dict, radius_km: float, top_k: int) -> list[dict]:
    ...
    if source.get("lat") is None or source.get("lng") is None:
        return []
    if not engine_state.available or engine_state.spatial is None:
        return []
...
def forecast_level(area_name: Optional[str], arrival_unix: int) -> int:
    """area 예보지점 T시점 혼잡 레벨(1~4). 엔진/예보지점 없으면 중립(2)."""
    if not engine_state.available or not area_name:
        return 2
    try:
