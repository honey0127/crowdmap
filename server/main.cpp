...
void handleClient(SOCKET clientSocket, int clientId) {
    ...
    try {
        // 클라이언트와의 지속적인 연결 유지 (여러 번 데이터 수신)
        while (true) {
            ...
            // 데이터 파싱: "userId,latitude,longitude" 형식
            ...
            std::cout << "[Client " << clientId << "] Zone count: " << zoneCount << "\n";
            
            // 3. 혼잡도 계산
            CongestionResult result = congestionCalculator.calculateCongestion(zoneCount);
            std::cout << "[Client " << clientId << "] Congestion: " << result.levelString() 
                      << " (" << result.ratio << ")\n";
            
            // 4. 응답 생성 및 전송
            std::string response = std::string(result.levelString()) + "|" + std::to_string(result.ratio);
...
int main() {
    ...
    // Firebase 초기화
    std::cout << "Initializing Firebase connection...\n";
    firebaseClient = std::make_unique<FirebaseClient>(FIREBASE_PROJECT_ID);
    
    // Firebase에서 데이터 로드 (시작 시 한 번)
    std::cout << "Loading places from Firebase...\n";
    auto places = firebaseClient->getPlaces();
    std::cout << "Loaded " << places.size() << " places\n";
    
    std::cout << "Loading zones from Firebase...\n";
